...
class TwoMinuteTokenGenerator(PasswordResetTokenGenerator):
    ...
    def check_token(self, user, token):
        ...
        print(f"Token received: {token}")  # Print the entire token for inspection
        ...
        try:
            ...
            print("Token Age:", token_age)  # Print the token's age for inspection
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if record.otp == input_otp and timezone.now() - record.created_at <= timedelta(minutes=2):
                auth_login(request, user)  # Log the user in
                record.delete()  # Cleanup the OTP record after successful login
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            if user is not None and user.is_active:
                ...
                LoginOTP.objects.update_or_create(user=user, defaults={'otp': otp, 'created_at': timezone.now()})
